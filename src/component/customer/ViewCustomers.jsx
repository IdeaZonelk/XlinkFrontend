...
function ViewCustomersBody() {
    ...
    useEffect(() => {
        if (keyword.trim() === '') {
            setSearchedCustomer(customerData);
            setError('');
        }
        // eslint-disable-next-line
    }, [keyword, customerData]);
            ...
            reader.onload = (e) => {
                try {
                    ...
                    const formattedData = json.map((row) => ({
                        ...
                        loyaltyReferenceNumber: row.LoyaltyReferenceNumber || row.loyaltyReferenceNumber || '',
                        redeemedPoints: row.RedeemedPoints || row.redeemedPoints || 0
                    }));
                    setExcelData(formattedData);
                    setSuccessStatus('File processed successfully.');
                } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            if (excelData.length > 0) {
                const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/importCustomers`, { customers: excelData });
                if (response.status === 201) {
                    toast.success('Customer imported successfully!', { autoClose: 2000 });
                    setTimeout(() => {
                        navigate("/viewCustomers");
                    }, 2000);
                    setExcelData([]);
                    ...
                    toast.error('Failed to save customers. Please try again.', { autoClose: 2000 });
                }
            } else {
                toast.error('No data to save. Please upload a valid file.', { autoClose: 2000 });
            }
        } catch (error) {
            setOpenPopup(false);
            if (error.response && error.response.data.message === 'Some customers already exist') {
                toast.error(`Customer(s) already exist: ${JSON.stringify(error.response.data.duplicates)}`, { autoClose: 2000 });
            } else {
                toast.error('Failed to save customers. Please try again.', { autoClose: 2000 });
            }
        }
    ...
    return (
        <div className='relative background-white absolute top-[80px] left-[18%] w-[82%] min-h-[100vh] p-5'>
                    ...
                    <table className="min-w-full bg-white border border-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Customer name</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Mobile</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Loyalty Reference Number</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Redeemed Points</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Created on</th>
                                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Action</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                            {searchedCustomer.map((customer) => (
                                <tr key={customer._id}>
                                    <td className="px-6 py-4 whitespace-nowrap text-m text-gray-900 text-left">{customer.name}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-m text-gray-900 text-left">{customer.mobile}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-m text-gray-900 text-center">{customer.loyaltyReferenceNumber}</td>
                                    <td className="px-6 py-5 whitespace-nowrap text-m text-gray-900 text-center">{customer.redeemedPoints}</td>
                                    <td className="px-8 py-4 whitespace-nowrap text-m text-gray-900 text-left">{customer.createdAt ? new Date(customer.createdAt).toLocaleDateString() : ''}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-m text-gray-900 justify-end">
                                        <div className='flex items-center'>
                                            {permissionData.edit_customer && (
                                                <Link to={`/editCustomerDetails/${customer._id}`}
                                                    className="text-blue-500 hover:text-blue-700 font-bold py-1 px-2 mr-2 flex items-center"
                                                    style={{ background: 'transparent' }}
                                                >
                                                    <i className="fas fa-edit mr-1"></i>
                                            ...
                                            {permissionData.delete_customer && (
                                                <button
                                                    onClick={() => showConfirmationModal(customer._id)}
                                                    className="text-red-500 hover:text-red-700 font-bold py-1 px-2 flex items-center"
                                                    style={{ background: 'transparent' }}
                                                >
                                                    <i className="fas fa-trash mr-1"></i>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {customerData.map((user) => (
                                <tr key={user._id}>
                                    <td className="px-6 py-4 whitespace-nowrap text-m text-gray-900 text-left">{user.name}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-m text-gray-900 text-left">{user.mobile}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-m text-gray-900 text-left">{user.loyaltyReferenceNumber}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-m text-gray-900 text-left">{user.redeemedPoints}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-m text-gray-900 text-left">{user.createdAt ? new Date(user.createdAt).toLocaleDateString() : ''}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-m text-gray-900 text-right justify-end">
                                        <div className='flex items-center justify-end'>
                                            {permissionData.edit_customer && (
                                                <Link to={`/editCustomerDetails/${user._id}`}
            ...
            <ConfirmationModal
                ...
                onConfirm={() => handleDelete(customerToDelete)}
                message="Are you sure you want to delete this customer?"
            />
                    ...
                    <div className="fixed inset-0 flex items-center justify-center z-50">
                        <div className="bg-white p-6 rounded-lg shadow-lg w-1/2 h-[450px] overflow-y-auto">
                            ...
                            <div className='mt-10'>
                                ...
                                <label className="block text-sm font-medium leading-6 text-gray-900 text-left">NIC : Required</label>
                                <label className="block text-sm font-medium leading-6 text-gray-900 text-left">Mobile : Required</label>
                                <label className="block text-sm font-medium leading-6 text-gray-900 text-left">Loyalty Reference Number : Required</label>
                                <label className="block text-sm font-medium leading-6 text-gray-900 text-left">Redeemed Points : Optional</label>
                            </div>
                            <div>
                                <button onClick={handleSave} className="submit flex-none rounded-md px-3.5 py-2.5 text-sm font-semibold text-white shadow-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 w-40 text-center">
                                    Save
