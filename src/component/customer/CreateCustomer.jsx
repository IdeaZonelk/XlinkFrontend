...
//                 setError('Email must be a valid email address containing "@".');
//                 setProgress(false);
//                 return;
//             }
            

//             // Mobile number validation: 10 digits starting with 0 (e.g., 0712345678)
//             const mobileRegex = /^0\d{9}$/;
//             if (!mobileRegex.test(mobile)) {
//                 setError('Invalid mobile number. It must start with "0" and contain exactly 10 digits.');
//                 setProgress(false);
//                 return;
//             }
...
//             if (nic &&!newNICRegex.test(nic) && !oldNICRegex.test(nic)) {
...
//             // Customer data
//             const customerData = {
//                 username: normalizedUsername || '', // optional
//                 name: name.trim(),
//                 nic: nic?.trim() || '',
//                 mobile: mobile.trim(),
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (!name || !nic || !mobile || !loyaltyReferenceNumber) {
                setError('All fields except Redeemed Points are required.');
                setProgress(false);
                return;
            }
            ...
            // Mobile number validation: 10 digits starting with 0
            const mobileRegex = /^0\d{9}$/;
            if (!mobileRegex.test(mobile)) {
                setError('Invalid mobile number. It must start with "0" and contain exactly 10 digits.');
                setProgress(false);
            ...
            if (!/^[a-zA-Z0-9]+$/.test(loyaltyReferenceNumber)) {
                setError('Loyalty reference must be alphanumeric.');
                setProgress(false);
                return;
            }
            ...
            if (redeemedPoints && isNaN(Number(redeemedPoints))) {
                setError('Redeemed Points must be a number.');
                setProgress(false);
                return;
            }
            ...
            const customerData = {
                ...
                nic: nic.trim(),
                mobile: mobile.trim(),
                loyaltyReferenceNumber: loyaltyReferenceNumber.trim(),
