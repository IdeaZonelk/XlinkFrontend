...
export const handleCustomerSearch = async (
  ...
  if (keyword.length > 0) {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/searchCustomerByName`,
        {
          params: { name: keyword },
        }
      );
  ...
  setSelectedProduct((prevProducts) => {
    ...
    if (product.ptype === "Variation" && product.variationValues) {
      ...
      const existingProductWithSameVariation = prevProducts.find(
        (p) =>
          p._id === product._id &&
          p.selectedVariation === Object.keys(product.variationValues)[0]
      );
      console.log(
        "Existing product with the same variation:",
        existingProductWithSameVariation
      );

      if (existingProductWithSameVariation) {
        alert(
          `The variation "${
            Object.keys(product.variationValues)[0]
          }" is already added.`
        );
        return prevProducts;
      }
...
export const handleVariationChange = (index, variation, setSelectedProduct) => {
  setSelectedProduct((prevProducts) =>
    prevProducts.map((product, i) => {
      if (i === index) {
        // Use index to find the correct product instance
        const productWithSameVariation = prevProducts.find(
          (p, j) =>
            j !== index &&
            p._id === product._id &&
            p.selectedVariation === variation
        );
        ...
        if (newPurchaseQty > stockQty) {
          alert(
            `Purchase quantity adjusted to the available stock (${stockQty}) for the "${variation}" variation.`
          );
...
export const getQty = (product, selectedVariation) => {
  ...
  if (product.variationValues && selectedVariation) {
    const variationQty = Number(
      product.variationValues[selectedVariation]?.productQty
    );
...
export const getProductCost = (product, selectedVariation) => {
  if (product.variationValues) {
    ...
    if (selectedVariation && product.variationValues[selectedVariation]) {
      const variationPrice = Number(
        product.variationValues[selectedVariation].productCost
      );
    ...
    prevProducts.map((product, i) => {
      if (i === index) {
        ...
        if (product.variationValues && selectedVariation) {
          ...
          if (newQty > stockQty) {
            alert(
              `Cannot exceed stock quantity of ${stockQty} for this variation.`
            );
        ...
        // For single products
          ...
          if (newQty > stockQty) {
            alert(
              `Cannot exceed stock quantity of ${stockQty} for this product.`
            );
            return product; // Prevent changing if stock limit is exceeded
          }
...
export const getTax = (product, selectedVariation) => {
  if (
    ...
    const variationTax = Number(
      product.variationValues[selectedVariation].orderTax
    );
...
export const handleSave = async (
  ...
  console.log(
    "[saleController] handleSave received claimedPoints:",
    claimedPoints
  );
  setResponseMessage("");
  ...
  let paidAmount = Object.values(amounts).reduce(
  ...
  if (normalizedPaymentStatus === "paid" && balance > 0) {
    alert(
      "Payment status is 'Paid', but there's still a balance remaining. Please adjust the payment amount."
    );
    setProgress(false);
    return;
  }
  ...
  const cashRegisterKey = cashierID
    ? cashierID
    : sessionStorage.getItem("cashierUsername");
  const cashRegisterID = RegisterID
    ? RegisterID
    : sessionStorage.getItem("cashRegisterID");
  const defaultWarehouse =
    sessionStorage.getItem("defaultWarehouse") || "Unknown";
  ...
  const productsData = selectedProduct.map((product) => {
    ...
    const applicablePrice = appliedWholesale
      ? wholesalePrice
      : product.price || getPriceRange(product, product.selectedVariation);
    const price =
      product.price || getPriceRange(product, product.selectedVariation);
    const productCost =
      product.producrCost || getProductCost(product, product.selectedVariation);
    const discount =
      product.discount || getDiscount(product, product.selectedVariation);
    ...
    const subtotal =
      (applicablePrice - discount - specialDiscount) * quantity +
      applicablePrice * quantity * taxRate;
    const productProfit =
      (applicablePrice - discount - specialDiscount) * quantity -
...
export const handleUpdateSale = async (
  ...
  const updatedPaymentStatus =
    paidAmount >= totalAmount ? "paid" : paymentStatus;
  ...
  const productsData = productData.map((product) => {
    ...
    const productCost = product.producrCost
      ? product.producrCost
  ...
  try {
    const response = await axios.put(
      `${process.env.REACT_APP_BASE_URL}/api/updateSale/${id}`,
      updatedSaleData
    );
    toast.success(
...
export const handleReturnSale = async (
  id,
  grandTotal,
  paidAmount,
  returnAmountDetail,
  ...
  if (typeof setLoading === "function") {
    ...
    console.warn(
      "setLoading is not a function. Skipping setLoading invocation."
    );
  }
  ...
  try {
    const response = await axios.post(
      `${process.env.REACT_APP_BASE_URL}/api/returnSale`,
      finalSaleData
    );
    toast.success(
...
export const handleUpdateSaleReturn = async (
  ...
  try {
    const response = await axios.put(
      `${process.env.REACT_APP_BASE_URL}/api/updateReturnSale/${id}`,
