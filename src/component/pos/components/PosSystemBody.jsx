...
import {
  handleFindProductBySearch,
  handleProductSubmit,
} from "../utils/searchProduct";
import {
  getHeldProducts,
...
function PosSystemBody({ defaultWarehouse }) {
  ...
  useEffect(() => {}, [productBillingHandling]);
      ...
      if (product.ptype === "Single") {
        ...
        if (existingProduct) {
          ...
          if (warehouseData.productQty > 0) {
            return [
              ...prevBilling,
              { ...product, qty: 1, warehouse: selectedWarehouse },
            ];
          } else {
  ...
  const handleRegisterReportOpen = async () => {
    ...
    const fetchReportData = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_BASE_URL}/api/findRegisterData`
        );
        if (response.data) {
  ...
  useEffect(() => {
    const fetchReportData = async () => {
      ...
      try {
        ...
        const { total, totalTaxAmount } = sales.reduce(
          (totals, sale) => {
            if (sale && sale.productsData) {
              ...
              if (sale.tax && sale.grandTotal) {
                saleTax =
                  (parseFloat(sale.tax) / 100) * parseFloat(sale.grandTotal);
              }
        ...
        const { grandTotal, totalDiscountAmount } = sales.reduce(
          (totals, sale) => {
            if (sale && sale.productsData) {
              let saleSubtotal = 0;
                            ...
                            <tr key={reg._id}>
                              ...
                              <td className="px-7 py-5 whitespace-nowrap text-m text-gray-900">
                                ...
                                <p className="rounded-[5px] text-center py-[6px] bg-green-100 text-green-500">
                                  {currency}{" "}
                                  {formatWithCustomCommas(reg.totalBalance)}
                                </p>
                              </td>
                            </tr>
                          ))}
                    ...
                    <div className="flex gap-2">
                      <div className="flex-1">
                        ...
                        <div className="relative">
                          ...
                          />
                          <span className="absolute rounded-l-lg left-[1px] p-3 bg-gray-100 top-1/2 transform -translate-y-1/2">
                            {currency}
                          </span>
                        </div>
                      </div>
                          ...
                          <div className="relative w-[170px]">
                            ...
                            <input
                              ...
                              value={formatWithCustomCommas(
                                Math.max(
                                  0,
                                  cashPaymentAmount - calculateTotal()
                                )
                              )}
                              readOnly
                              className="w-full border border-gray-300 px-3 py-2 pl-10 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-[#35AF87]"
              ...
              {!selectedWarehouseAccess ? (
                ...
                >
                  {(searchedProductDataByName.length > 0
                    ...
                    const productQtyForSelectedWarehouse =
                      ...
                      >
                        ...
                        <div className="flex space-between items-center text-left mt-[2px]">
                          <p className="bg-blue-600 mr-1 text-left px-1 py-[1.5px] rounded-[5px] text-center text-[11px] text-white">
                            {productQtyForSelectedWarehouse}
                            {" " + p.saleUnit}
                          </p>
