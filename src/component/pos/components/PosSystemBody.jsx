...
function PosSystemBody({ defaultWarehouse }) {
  ...
  const combinedProductData =
    showServicesData && services.length > 0
      ? services
  ...
  const handleAddingProduct = (product) => {
    ...
    setProductBillingHandling((prevBilling) => {
      ...
      if (product.isService) {
        ...
        console.log("Service price:", product.price);
        const existingService = prevBilling.find(
          (p) => p.id === product.id
        );

        if (existingService) {
          console.log("Updating existing service quantity");
          ...
          const serviceForBilling = { 
            ...product, 
            qty: 1,
            stokeQty: 999999, // Services have unlimited quantity
            isService: true,
            currentID: product.id, // Add currentID for payment processing
            productQty: 999999 // Add productQty for payment processing
          };
          console.log("✅ Adding new service to billing:", serviceForBilling);
          return [...prevBilling, serviceForBilling];
        }
      } else {
        console.log("🛍️ PROCESSING REGULAR PRODUCT:", product.name);
      }
  ...
  const fetchServices = async () => {
    try {
      setProgress(true);
      console.log('Fetching services with warehouse:', warehouse);
      const response = await fetch(`${process.env.REACT_APP_BASE_URL}/api/findAllService`);
      const data = await response.json();
      console.log('Raw service response:', data);
      console.log('Response keys:', Object.keys(data));
      console.log('Services array check:', Array.isArray(data.services));
      console.log('Status check:', data.status);
      
      if (Array.isArray(data.services) && data.services.length > 0) {
        // Transform services to match product structure for billing
        const transformedServices = data.services.map(service => ({
          _id: service._id,
          productName: service.serviceName,
          serviceName: service.serviceName,
          name: service.serviceName, // For display
          code: `SRV-${service._id.slice(-6)}`, // Generate service code
          ...
          saleUnit: 'Service', // Unit type for services
          isService: true, // Flag to identify this as a service
          ptype: 'simple', // Product type
          taxType: 'exclusive',
          tax: [{
            taxPercentage: 0,
            taxType: 'exclusive'
          ...
          warehouseData: [{
            ...
            qty: 999999, // Services have unlimited quantity
            productQty: 999999
          }],
          warehouse: {
            [warehouse || 'default']: {
              productPrice: service.price || 0, // Add price here for getPriceRange function
              productQty: 999999,
              orderTax: 0, // Add tax for getTax function
              discount: 0, // Add discount for getDiscount function
              taxType: 'exclusive',
              variationValues: {},
              wholesaleEnabled: false,
              wholesaleMinQty: 0,
              wholesalePrice: 0,
              productCost: service.price || 0 // Add cost for getProductCost function
            }
          }
        }));
        setServices(transformedServices);
        console.log('Transformed services:', transformedServices);
        console.log('Services set successfully, length:', transformedServices.length);
      } else {
        console.log('Services condition failed - Status:', data.status, 'IsArray:', Array.isArray(data.services));
        setServices([]);
      }
    } catch (error) {
  ...
  const handleRealTimeSearch = async (searchTerm) => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/findProductByName`,
        {
          params: {
            keyword: searchTerm,
            warehouse: warehouse,
              ...
              <div className="flex space-x-2">
                ...
                {loading ? (
                  ...
                  <>
                    ...
                    <button
                      onClick={() => {
                        console.log('Services button clicked, current state:', showServicesData);
                        setShowServicesData(!showServicesData);
                        setSelectedBrand(null);
                        ...
                        if (!showServicesData) {
                          ...
                          console.log('Hiding services');
                        }
                      }}
                      className={`flex-shrink-0 border rounded-lg px-4 py-2.5 flex flex-col items-center justify-center hover:shadow-md transition-colors ${
                        showServicesData
                          ? "custom text-white border-blue-500"
                          : "bg-blue-100 text-blue-800 border-blue-200 hover:bg-blue-200"
                      }`}
                    >
                      <h3 className="text-center text-m font-medium">Services</h3>
                    </button>
                  </>
                )}
              </div>
